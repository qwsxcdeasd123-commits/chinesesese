...
<style>
    ...
    .header {
        ...
        border-radius: 0;
        margin: -1rem -1rem 0.25rem -1rem !important; /* 위치 수정 */
    ...
    .assistant-message {
        ...
        float: left;
        clear: both;
        box-shadow: none !important;             /* 선택: 볼륨감 제거 */
        cursor: pointer;
        word-wrap: break-word;
    ...
    .analysis-content { padding: 1rem; background: #ffffff; }
    .analysis-section { margin-bottom: 1rem; }
    .analysis-label { font-size: 0.75rem; color: #999999; margin-bottom: 0.375rem; font-weight: 500; }
    ...
    .grammar-box { background: #fef9e7; padding: 0.625rem; border-radius: 0.25rem; font-size: 0.8125rem; color: #333333; border: 1px solid #fde68a; line-height: 1.5; }
    .vocabulary-box { background: #f0fdf4; padding: 0.625rem; border-radius: 0.25rem; font-size: 0.8125rem; color: #333333; border: 1px solid #bbf7d0; line-height: 1.5; }
    .notes-box { background: #fef3c7; padding: 0.625rem; border-radius: 0.25rem; font-size: 0.8125rem; color: #333333; border: 1px solid #fde68a; line-height: 1.5; }
    .feedback-box { background: #f3e8ff; padding: 0.625rem; border-radius: 0.25rem; font-size: 0.8125rem; color: #333333; border: 1px solid #d8b4fe; line-height: 1.5; }
    ...
    .stTextInput > div > div > input:disabled { background: #f5f5f5; color: #999999; cursor: not-allowed; }
    
    .stButton { margin-bottom: 0 !important; }
    .stButton > button[kind="primary"] {
        background: #09b83e; color: white; border: none; border-radius: 50%;
    ...
    [data-testid="stSidebar"] .stSelectbox > div > div { background: #ffffff; border: 1px solid #e5e5e5; border-radius: 0.375rem; }
...
def _build_tutor_system_prompt(target_lang: str):
    return (
        ...
        "- 이후부터는 목표 재언급 금지, 저장된 사용자 이름을 존칭으로 호명(예: 미주님).\n"
    )
...
def generate_assistant_reply(user_msg: str):
    ...
    if is_first_turn:
        user_instruction = (
            ...
            "이후부터는 목표를 반복하지 말고, 이름이 저장되면 존칭으로 호명하여 대화하라."
        )
    else:
        user_instruction = (
            f"{name_clause}\n"
...
def extract_user_name_from_message(latest_user_msg: str) -> str:
    """LLM으로 이름만 추출. 불명확하면 빈 문자열 반환."""
    system_prompt = (
        "역할: 정보 추출기.\n"
        "규칙: 입력 문장에서 사람 이름만 한국어 표기 그대로 추출. "
        "이름이 명시되지 않았으면 빈 문자열을 반환. 설명 금지. JSON만 출력."
    )
    user_prompt = (
...
def generate_chinese_analysis(user_msg: str):
    ...
    system_prompt = (
        ...
        "모든 한국어 설명은 간결하고 정확하게. 불확실하면 '확인 불가' 표기."
    )
    user_prompt = (
        "다음 학습자 발화를 분석하라.\n"
        f"발화: {user_msg}\n"
        "형식은 JSON만 반환하고, 추가 설명은 금지."
    )
    raw = _claude(messages=[{"role":"user","content":user_prompt}], system=system_prompt, max_tokens=900, temperature=0)
    try:
        data = json.loads(raw)
        # words 매핑
        words = []
...
if st.session_state.selected_language == 'chinese' and st.session_state.detailed_analysis:
    with st.expander("📚 상세 분석", expanded=st.session_state.show_analysis):
        ...
        if analysis.get('feedback'):
            ...
            """, unsafe_allow_html=True)
            st.markdown(f"<div style='margin-bottom: 0.5rem;'><strong>表现 (표현):</strong> {fdb.get('expression','확인 불가')}</div>", unsafe_allow_html=True)
            st.markdown(f"<div style='margin-bottom: 0.5rem;'><strong>语法 (문법):</strong> {fdb.get('grammar_feedback','확인 불가')}</div>", unsafe_allow_html=True)
            st.markdown(f"<div style='margin-bottom: 0.5rem;'><strong>语境 (맥락):</strong> {fdb.get('context','확인 불가')}</div>", unsafe_allow_html=True)
            st.markdown(f"<div><strong>单词选择 (단어 선택):</strong> {fdb.get('word_choice','확인 불가')}</div>", unsafe_allow_html=True)
...
if st.session_state.is_loading and len(st.session_state.messages) > 0 and st.session_state.messages[-1]['role'] == 'user':
    ...
    try:
        ...
        if not reply_text:
            reply_text = "확인 불가"
        # 이름 저장이 아직 없고 LLM이 이름을 물었다면, 다음 사용자 입력에서 캡처됨
        # 이름이 이미 있으면 존칭 호명은 LLM 프롬프트 정책에 의해 적용됨
