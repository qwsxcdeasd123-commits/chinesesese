...
def call_claude(messages, system, max_tokens=800, temperature=0):
    ...
    # resp.content: list[ContentBlock]; text는 .text
...
<style>
    ...
    .header {
        ...
        border-radius: 0;
        margin: -1rem -1rem 0.25rem -1rem !important; /* !important 위치 수정 */
    ...
    .messages-container {
        background: #ededed;
        min-height: 200px; /* 빈 화면 때 과도한 하강 방지 */
        max-height: 550px;
        overflow-y: auto;
    ...
    .empty-state {
        text-align: center;
        padding: 1rem 1rem !important; /* !important 위치 수정 */
    }
...
def build_system_prompt(target_lang: str):
    return (
        ...
        "규칙:\n"
        "- 감정 표현 금지.\n"
        "- 사실 기반으로만 답변, 불확실한 내용은 '확인 불가' 명시.\n"
        "- 추측 금지, 간결한 문장 사용.\n"
        "- 출력 언어: 사용자가 선택한 학습 언어로 답변.\n"
        "- 첫 응답에서만 학습 목표를 1회 간략히 언급하고 이후 반복 금지.\n"
    )
...
def generate_assistant_reply(user_msg: str):
    ...
    if is_first_turn:
        user_instruction = (
            ...
            f"사용자 입력: {user_msg}\n"
            "첫 응답이므로 학습 목표를 1회만 간략히 언급하고, 이후에는 언급하지 마십시오."
        )
    else:
        user_instruction = (
            f"사용자 입력: {user_msg}\n"
            "학습 목표는 재언급 금지. 질문에 직접 간결히 답변."
        )
...
def generate_chinese_analysis(user_msg: str):
    ...
    system_prompt = (
        ...
        "설명은 간결하게, 한국어 해설은 meaning_ko에 표기."
    )
    user_prompt = (
        "다음 학습자 발화를 분석하라.\n"
        f"발화: {user_msg}\n"
        "형식은 JSON만 반환하고, 불필요한 텍스트 금지."
    )
